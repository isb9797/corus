...
<template>
  <div class="ask-form" id="ask-form">
    <div class="ask-form__header">
      ...
      <router-link to="/results" class="ask-form__header-all">
...
<!--    Сами инпуты тоже можно поместить в отдельные компоненты-->
    <form action="#">
      <div class="checkbox-control" v-for="ask in d">
        <input type="checkbox" v-model="checkedData" :value=ask.title :name=ask.id :id=ask.id>
        <label :for=ask.id>{{ask.title}}</label>
      </div>


<!--        <span v-if="checkedData.length >= 1">Отмеченные варианты: {{ checkedData }}</span>-->
